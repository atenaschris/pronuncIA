...
import { LANGUAGE_LEVELS, LEARNING_GOALS, LEARNING_STYLES, NATIVE_LANGUAGES, TARGET_LANGUAGES, TIME_OPTIONS } from '@/lib/constants/constants';
...
export const useOnboardingStore = create<OnboardingState>()(persist(
  (set) => ({
    ...
    targetLanguage: TARGET_LANGUAGES[0].id, // Default to English
    ...
    resetOnboarding: () => set({
      ...
      languageLevel: LANGUAGE_LEVELS[0].id,
      nativeLanguage: NATIVE_LANGUAGES[0].id,
      targetLanguage: TARGET_LANGUAGES[0].id,
      learningGoal: LEARNING_GOALS[0].id,
      timeCommitment: "15",
      learningStyle: LEARNING_STYLES[0].id,
      isComplete: false,
